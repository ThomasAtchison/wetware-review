...
#import "RecentPartsController.h"

#define kCellIdentifier @"edu.stanford.bioeng.endylab.BioBrickStudio.RecentPartsSelectionCellIdentifier"

@implementation RecentPartsController
...
{	
	...
	url = [[NSURL alloc] initWithString:@"http://partsregistry.org/das/parts/recent/index.cgi?count=100"];
	...
	@try
	{
		...
		[parser parse];
		[_parts sortUsingDescriptors:[NSArray arrayWithObject:[[NSSortDescriptor alloc] initWithKey:@"type" ascending:YES]]];
	}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	text = [[_parts objectAtIndex:[indexPath row]] type];
	[cell setText:text];
...
{	
	...
	if([elementName caseInsensitiveCompare:@"SEGMENT"] == NSOrderedSame)
	{
		...
		[part setType:[attributeDict valueForKey:@"part_type"]];
		[part setSize:[NSNumber numberWithInt:[[attributeDict valueForKey:@"size"] intValue]]];
//		[part setShortDescription:[[attributeDict valueForKey:@"short_desc"] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]];
		[part setShortDescription:[NSString stringWithFormat:@"Short description for %@",[attributeDict valueForKey:@"id"]]];
